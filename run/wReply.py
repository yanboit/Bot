...
def main(bot, logger):
    ...
    async def startAddRep(event: GroupMessage):
        ...
        if str(event.message_chain) == "开始添加":
            await sleep(0.1)
            operateProcess[event.sender.id] = {"status": "startadd", "time": datetime.datetime.now(),
        ...
        if event.sender.id in operateProcess:
            if operateProcess[event.sender.id]["status"] == "startadd":
                ...
                if operateProcess[event.sender.id]["operateId"] in publicDict:
                    r = await getRep(publicDict.get(operateProcess[event.sender.id]["operateId"]),
                                     event.message_chain.json(), wReply.get("threshold"), wReply.get("mode"),
                                     wReply.get("inMaxLength"), wReply.get("inWeighting"))
        ...
        if event.sender.id in operateProcess:
            if operateProcess[event.sender.id]["status"] == "adding":
                if str(event.message_chain) == "over":
                    ...
                    publicDict = await addRep(operateProcess[event.sender.id]["key"],
                                              operateProcess[event.sender.id]["value"],
                                              operateProcess[event.sender.id]["operateId"])
    ...
    async def queryValue(event: GroupMessage):
        ...
        if str(event.message_chain) == "查回复":
            await sleep(1)
            operateProcess[event.sender.id] = {"status": "query", "operateId": str(event.group.id),
                                               "time": datetime.datetime.now()}
        elif str(event.message_chain) == "*查回复":
            await sleep(1)
            operateProcess[event.sender.id] = {"status": "query", "operateId": "publicLexicon",
                                               "time": datetime.datetime.now()}
        else:
            return
        logger.info("查询回复目标")
    ...
    async def sendQueryResults(event: GroupMessage):
        ...
        if event.sender.id in operateProcess:
            ...
            if operateProcess[event.sender.id]["status"] == "query":
                r = await getRep(publicDict.get(operateProcess[event.sender.id]["operateId"]),
                                 str(event.message_chain.json()), wReply.get("threshold"), wReply.get("mode"),
                                 wReply.get("inMaxLength"), wReply.get("inWeighting"))
                b1 = []
                if r != None:
                    index = 0
            ...
            if operateProcess[event.sender.id]["status"] == "delete":
                if str(event.message_chain).startswith("删除#"):
                    try:
                        ...
                        value = publicDict.get(operateProcess[event.sender.id]["operateId"]).get(
                            operateProcess[event.sender.id]["queryKey"])
                        value.pop(index)
                        publicDict = await addRep(operateProcess[event.sender.id]["queryKey"], value,
                                                  operateProcess[event.sender.id]["operateId"])
                    ...
                    try:
                        publicDict[operateProcess[event.sender.id]["operateId"]].pop(
                            operateProcess[event.sender.id]["queryKey"])
                        ...
                        with open(id, 'w', encoding="utf-8") as file:
                            yaml.dump(publicDict[operateProcess[event.sender.id]["operateId"]], file,
                                      allow_unicode=True)
                    except:
                        logger.error("非法输入，索引必须是有效的数字")
                else:
                    if str(event.message_chain) == "over":
                        publicDict = loadAllDict()
                        operateProcess = {}
                        await bot.send(event, "已退出", True)
                        return
                    publicDict = loadAllDict()
                    operateProcess = {}
                    await bot.send(event, "非法输入，结束删除", True)
                    return
    ...
    async def getReply(event: GroupMessage):
        ...
        if str(event.group.id) in publicDict:
            r = await getRep(publicDict.get(str(event.group.id)), str(event.message_chain.json()),
                             wReply.get("threshold"), wReply.get("mode"), wReply.get("inMaxLength"),
                             wReply.get("inWeighting"))
        else:
            r = await getRep(publicDict.get("publicLexicon"), str(event.message_chain.json()), wReply.get("threshold"),
                             wReply.get("mode"), wReply.get("inMaxLength"), wReply.get("inWeighting"))
        if r != None:
    ...
    async def repeatFunc(event: GroupMessage):
        ...
        if event.group.id in repeatData:
            ...
            if event.group.id in repeatLock and datetime.datetime.now() - repeatLock[
                event.group.id] < datetime.timedelta(60):  # 时间锁
                return
            try:
                repeatData[event.group.id] = {"times": 1, "mes": event.message_chain.json(),
                                              "simplemes": str(event.message_chain),
                                              "sefmeschain": await EventMessageConvert(event.message_chain, True)}
            except:
                pass
